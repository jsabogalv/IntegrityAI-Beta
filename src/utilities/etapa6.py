...
def procesar_y_normalizar_archivos(excel_path, input_data, master_db_path, output_dir):
    ...
    for key, value in input_data.items():
        if key.startswith("Datos Libres -") and value:
            # Obtener el nombre del input seleccionado
            selected_input = key.replace("Datos Libres - ", "").strip()

            # Buscar en la columna A el nombre seleccionado y obtener el nombre del archivo en la columna E
            matching_rows = data[data.iloc[:, 0].str.strip() == selected_input]
            if not matching_rows.empty:
                file_names = matching_rows.iloc[:, 4].dropna().tolist()
    ...
    if archivos_a_normalizar:
        ...
        output_file_path = os.path.join(output_dir, f'{next_number:02d}_DatosLibres.parquet')
    ...
    log_path = os.path.join(output_dir, f'{next_log_number:02d}_DatosLibres_log.txt')
...
if __name__ == "__main__":
    if len(sys.argv) >= 4:
        ...
        master_db_path = os.path.join(os.path.dirname(__file__), '../../resources/Master/DB')
        output_dir = os.path.join(project_dir, 'DataSet', 'Entrenamiento')
        
        procesar_y_normalizar_archivos(excel_path, input_data, master_db_path, output_dir)
    else:
        print("Uso: etapa6.py <project_dir> <input_data_dict> <excel_path>")
